...
import time
import serial, os, rospy

test = True

class DriveEsc:

    global test # test checks if test mode enabled
	...
	# ros display info
	return str(num), str(int(127*speed/100)), self.send(cmd, int(127*speed/100))
    ...
    def driveBoth(self, speed1, speed2):
        if speed1 != 0 and speed2 in range(-10, 10):
            self.drive(1, speed1)
            self.drive(2, 0)
	    return str(speed1) , str('0')
        else:
            self.drive(1, speed1)
            self.drive(2, speed2)
	    return str(speed1), str(speed2)
